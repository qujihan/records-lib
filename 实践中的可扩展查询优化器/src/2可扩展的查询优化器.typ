...
在这一章中，我们首先介绍一个扩展的优化器的概念（@可扩展的查询优化器基本概念）。然后我们深入讨论两个可以扩展的优化器框架：Volcano框架以及Cascades框架。我们从介绍Volcano以及其搜索开始（@volcano）, 然后我们简单的看下Volcano的局限性。正因其局限性，催生了其后来者: Cascades框架（@cascades）。我们介绍了在实践中用于提高Volcano和Cascades效率的其他优化以及启发式方法（@提高查询效率的技术）。为了说明可扩展的查询优化器如何轻松的将新功能合并到查询处理中，我们介绍了Microsoft SQL Server的优化器是如何利用Cascades框架的可扩展性来实现列存的（@Microsoft-SQL-Server的扩展性示例）。在本章的最后，我们将介绍可扩展的查询优化器是如何生成多核并行以及分布式的查询（@并行分布式查询流程）。
...
// TODO: 补全
*逻辑以及物理属性* 举一个表达式的逻辑属性的例子，就如表达式的关系代数表达式以及表达式的_基数（cardinality）_。例如$A join B$和$B join A$都产生了A与B join的结果，且它们拥有相同的逻辑数据（例如基数）

// TODO: 补全
*表达式的等价性* 如果两个表达式的属性是相同的，那么就认为这两个表达式是等价的。例如@query2的逻辑以及物理计划\表达式$L_1: A join B$与$L_2: B join A$是等价的。如果两个物理表达式的逻辑以及物理属性是相同的，那么也认为这两个物理表达式是等价的。例如下面两个表达式就认为是等价的。
